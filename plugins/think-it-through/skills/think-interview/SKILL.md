...
**Default target:** The smallest current subject with important missing information.
...
**Runs for:** Multiple turns. Keep the selected target until understanding is sufficient or the user stops, redirects, or invokes another command.
**Limits:** Stay neutral. Do not challenge, recommend, or turn the interview into a grill.
**Combines with:** A selector applies the target for the full interview. Modifiers can represent the final understanding without changing it.

## Flow

```mermaid
flowchart TD
    A["Selected target"] --> B["Reconstruct what is known"]
...
At launch, show the full trace: `> 🎯 **<target>** → 🔎 **INTERVIEW**`. On later turns, show `> 🔎 **INTERVIEW** · <target>`.
